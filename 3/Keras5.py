...
def create_googlenet(weights_path=None):
    ...
    loss3_classifier = Dense(9,name='loss3/classifier',W_regularizer=l2(0.0002))(pool5_drop_7x7_s1)
...
for time in range(times):
    ...
    Y = numpy.zeros((len(lines), 1000))
    for i in range(len(Y)):
        Y[i, label[i]] = 1

    image_train, image_test, label_train, label_test = train_test_split(image, Y, test_size=0.1, random_state=1)

    model.fit(image_train, [label_train, label_train, label_train], batch_size=40, nb_epoch=4, validation_data=(image_test, [label_test, label_test, label_test]), callbacks=[checkpointer])

    score = model.evaluate(image_test, [label_test, label_test, label_test], verbose=0)
